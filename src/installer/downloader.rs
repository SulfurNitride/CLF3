...
/// Report archive completion for aggregate download counters.
        ...
        match result {
            ...
            Err(e) => {
                ...
                if let Some(source_url) = get_proxy_source_url(state) {
                    let progress_callback = make_progress_callback(handle);
...
/// Create a download progress callback that updates the reporter handle.
fn make_progress_callback(handle: &Arc<dyn ProgressHandle>) -> Option<HttpProgressCallback> {
    ...
    loop {
        ...
        tokio::select! {
            ...
            nxm_result = rx.recv() => {
                ...
                let progress_callback = make_progress_callback(&dl_handle);
            ...
            Some((hash, name, output_path, result)) = result_rx.recv() => {
                ...
                nxm_completed += 1;
                reporter.status(&format!("Completed {} ({}/{})", name, nxm_completed, total_nexus));
