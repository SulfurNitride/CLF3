...
impl CollectionDb {
    ...
    fn create_tables(&self) -> Result<()> {
        self.conn
            ...
            CREATE TABLE IF NOT EXISTS mods (
                ...
                hashes_json TEXT,
                -- Vortex bsdiff patches map: { "<rel_path>": "<crc32_hex>" }
                patches_json TEXT,
    ...
    ///
    /// Idempotent: clears any existing rows first so reruns against an
    /// existing `collection.db` produce a fresh import (instead of stacking
    /// duplicates on top of prior runs).
    pub fn import_collection(&mut self, collection: &Collection) -> Result<()> {
        ...
        .context("Failed to clear collection tables before import")?;
        ...
        // Import mods
        {
            ...
            for mod_entry in &collection.mods {
                ...
                // Store bsdiff patches map (path → CRC32 hex) if present.
                let patches_json = mod_entry
                    .patches
                    .as_ref()
                    .filter(|m| !m.is_empty())
...
pub struct ModDbEntry {
    ...
    /// Vortex bsdiff patches map JSON: { "<rel_path>": "<crc32_hex>" }
...
impl ModDbEntry {
    ...
    /// Get bsdiff patches map (file path → CRC32 hex of expected source).
    pub fn get_patches(&self) -> Option<std::collections::HashMap<String, String>> {
        self.patches_json
            .as_ref()
