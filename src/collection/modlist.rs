...
pub struct ModInfo {
    ...
    pub phase: i32,
    /// Vortex `fileOverrides` non-empty → bump priority above peers.
    /// Approximates Vortex's deployment-engine override semantics under
    /// MO2's overlay model.
    pub has_file_overrides: bool,
}
...
impl ModListGenerator {
    ...
    pub fn generate_mod_order_combined(
        ...
        // up at the BOTTOM of modlist after reverse. So bigger tuple values
        // = popped later = top of modlist.
        //
        // `override_rank`: 1 when the mod has Vortex `fileOverrides` set.
        // It comes *after* `no_plugin` so override semantics apply within
        // each class (asset-only mods vs plugin mods) instead of pulling
        // override ESPs above asset overrides indiscriminately.
        let tie_breaker: Vec<(i32, i32, i32, i32, i32)> = (0..n)
            .map(|i| {
                let pos = mod_plugin_pos[i];
                let no_plugin = if pos == i32::MAX { 1 } else { 0 };
                let override_rank = if mods[i].has_file_overrides { 1 } else { 0 };
                (no_plugin, override_rank, pos, mods[i].phase, i as i32)
            })
            .collect();
    ...
    pub fn write_modlist(path: &Path, mod_order: &[String]) -> Result<()> {
        ...
        writeln!(file, "# This file was automatically generated by clf3")?;
